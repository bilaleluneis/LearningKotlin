...
/**
 * and infix extension function on basics.Person Type
...
infix fun Person.doesntLiveIn(address: Address?) : Boolean {
    ...
    //val liveInSameAddress = this.basics.livesIn(address)
...
fun returnNonNulItemlList(aList: List<String?>) : List<String> {
    ...
    for(item in aList){
        ...
        //or can be called like this too .. item?.let{result = result.basics.plus(item)}
    }
